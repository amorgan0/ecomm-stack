"""Rate calculation module with carrier-specific pricing."""
import logging
import asyncio
from typing import Dict, List, Optional

from app.services.carrier_base import AddressInfo, PackageInfo, RateQuote, ServiceLevel
...
class RateCalculator:
    """Calculate shipping rates across multiple carriers."""
    ...
    def __init__(self, carriers: Optional[List[str]] = None):
        ...
        if carriers:
            self._carriers = {k: v for k, v in self._carriers.items() if k in carriers}
    ...
    def calculate_rates(
        ...
        for carrier_code, client in self._carriers.items():
            try:
                rates = self._calculate_carrier_rates(client, origin, destination, packages, service_level)
    ...
    def _calculate_carrier_rates(
        ...
        if loop.is_running():
            ...
            return loop.run_until_complete(client.get_rates(origin, destination, packages, service_level))
