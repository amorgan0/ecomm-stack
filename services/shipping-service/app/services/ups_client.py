...
class UPSClient(CarrierClient):
    ...
    DIM_DIVISOR = 139  # UPS DIM divisor (same as FedEx)
    
    # Service code mappings (UPS uses numeric codes)
    SERVICE_CODES = {
        ServiceLevel.GROUND: ["03"],  # UPS Ground
        ServiceLevel.EXPRESS: ["12", "02"],  # 3 Day Select, 2nd Day Air
        ServiceLevel.OVERNIGHT: ["01", "14"],  # Next Day Air, Next Day Air Early
    }
    ...
    def __init__(self):
        """Initialize UPS client with credentials."""
    ...
    def _calculate_fuel_surcharge(self, base_cost: float) -> float:
        """Calculate fuel surcharge (currently ~10% for UPS)."""
        fuel_rate = 0.10  # 10% fuel surcharge
        return round(base_cost * fuel_rate, 2)
    
    def _calculate_hundredweight_discount(self, total_weight: float) -> float:
        """Calculate hundredweight discount for bulk shipments."""
        if total_weight >= 100:
            return 0.05  # 5% discount for shipments over 100 lbs
        elif total_weight >= 50:
            return 0.02  # 2% discount for shipments over 50 lbs
        return 0.0
    ...
    def calculate_rate(
        ...
        """Calculate UPS rate using zone-based pricing and dimensional weight."""
        ...
        service_codes = self.SERVICE_CODES.get(service_level, ["03"])
        service_names = {
    ...
    async def get_rates(
        ...
        """Get UPS shipping rates for all service levels."""
        rates = []
    ...
    def get_service_codes(self, service_level: ServiceLevel) -> List[str]:
        """Get UPS service codes for a service level."""
